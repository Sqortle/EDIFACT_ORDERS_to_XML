    ...
    def convert(self, edifact_message):
        ...
        for seg in segments:
            ...
            if tag == "NAD":
                ...
                party_id = self.sanitize(parts[2].split(":")[0])
                ...
                ET.SubElement(schedule, "MESSAGE_ID").text = self.sanitize(parts[2])
                ...
                ET.SubElement(schedule, "SUPPLIER_REF").text = self.sanitize(parts[1].split(":")[1])
        ...
        for seg in segments:
            ...
            if tag == "LIN":
                ...
                ean_code = self.sanitize(parts[3].split(":")[0]) if len(parts) > 3 and ":" in parts[3] else ""
                ET.SubElement(current_article_line, "EAN_CODE").text = ean_code
                ET.SubElement(current_article_line, "CALL_OFF_NO").text = self.sanitize(schedule.findtext("MESSAGE_ID", ""))
                ET.SubElement(current_article_line, "LAST_RECEIPT_DATE").text = self.sanitize(schedule.findtext("VALID_UNTIL", ""))
                ...
                ET.SubElement(current_demand_line, "DOCK_CODE").text = self.sanitize(schedule.findtext("EAN_LOCATION", ""))
                ET.SubElement(current_demand_line, "SCHEDULE_NO").text = schedule_no_str
                ET.SubElement(current_demand_line, "QUANTITY_DUE").text = "0" 
                ET.SubElement(current_demand_line, "DELIVERY_DUE_DATE").text = self.sanitize(schedule.findtext("VALID_UNTIL", ""))
                ET.SubElement(current_demand_line, "CUSTOMER_PO_NO").text = self.sanitize(schedule.findtext("MESSAGE_ID", ""))
                ET.SubElement(current_demand_line, "TO_DATE").text = self.sanitize(schedule.findtext("VALID_UNTIL", ""))
                ...
                if len(parts) > 2 and ":" in parts[2]:
                    code, qual = self.safe_split_component(parts[2])
                    if qual == "BP":
                        ET.SubElement(current_article_line, "PART_NO").text = self.sanitize(code)
                        pn = last_demand_line.find("PART_NO")
                        if pn is not None:
                            pn.text = self.sanitize(code)

            elif tag == "IMD" and current_article_line is not None:
                desc = self.sanitize(parts[3].replace(":::", "")) if len(parts) > 3 else ""
                ...
                qty_val = "0"
                try:
                    qty_val = self.sanitize(parts[1].split(":")[1])
                except Exception:
                    qty_val = "0"
                ...
                price_val = "0"
                try:
                    price_val = self.sanitize(parts[1].split(":")[1])
                except Exception:
                    price_val = "0"
